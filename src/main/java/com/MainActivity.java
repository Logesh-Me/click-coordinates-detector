...
public class MainActivity extends AppCompatActivity {
	...
	private Integer OVERLAY_PERMISSION_REQ_CODE = 1234;
	...
	private static boolean isServiceEnabled(Context context) {
		...
		if (accessibilityManager != null) {
			...
			for (AccessibilityServiceInfo serviceInfo : accessibilityManager.getEnabledAccessibilityServiceList(
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == OVERLAY_PERMISSION_REQ_CODE) {
			if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
				if (!Settings.canDrawOverlays(this)) {
					// You can show a warning message to the user that the permission is not granted
				} else {
